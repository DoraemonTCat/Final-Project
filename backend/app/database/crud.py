...
def update_customer_mining_status(db: Session, page_id: int, customer_psid: str, status: str):
    """อัพเดทสถานะการขุดของลูกค้า"""
    customer = get_customer_by_psid(db, page_id, customer_psid)
    ...
    if customer:
        ...
        if status not in valid_statuses:
            logger.error(f"Invalid mining status: {status}")
            return None
            
        customer.mining_status = status
        customer.updated_at = datetime.now()
        
        db.commit()
        db.refresh(customer)
        
        logger.info(f"Updated mining status for {customer_psid} to {status}")
        return customer
    
    return None

def bulk_update_mining_status(db: Session, page_id: int, psids: List[str], status: str):
    """อัพเดทสถานะการขุดแบบหลายคนพร้อมกัน"""
    try:
        # Validate status
        valid_statuses = ['not_mined', 'mined', 'responded']
        if status not in valid_statuses:
        ...
        updated_count = db.query(FbCustomer).filter(
            FbCustomer.page_id == page_id,
            FbCustomer.customer_psid.in_(psids)
        ).update(
            {
                FbCustomer.mining_status: status,
                FbCustomer.updated_at: datetime.now()
            },
            synchronize_session=False
        )
        
        db.commit()
        logger.info(f"Updated mining status for {updated_count} customers to {status}")
        return updated_count
        
    except Exception as e:
        db.rollback()
        logger.error(f"Error bulk updating mining status: {e}")
        return 0

def get_customers_by_mining_status(db: Session, page_id: int, status: str, skip: int = 0, limit: int = 100):
    """ดึงลูกค้าตามสถานะการขุด"""
    return db.query(FbCustomer).filter(
        FbCustomer.page_id == page_id,
        FbCustomer.mining_status == status
    ).order_by(FbCustomer.last_interaction_at.desc()).offset(skip).limit(limit).all()

def get_mining_statistics(db: Session, page_id: int):
    """ดึงสถิติการขุดของเพจ"""
    not_mined = db.query(func.count(FbCustomer.id)).filter(
        FbCustomer.page_id == page_id,
        FbCustomer.mining_status == 'not_mined'
    ).scalar()
    
    mined = db.query(func.count(FbCustomer.id)).filter(
        FbCustomer.page_id == page_id,
        FbCustomer.mining_status == 'mined'
    ).scalar()
    
    responded = db.query(func.count(FbCustomer.id)).filter(
        FbCustomer.page_id == page_id,
        FbCustomer.mining_status == 'responded'
    ).scalar()
    
    total = not_mined + mined + responded
    ...
    return {
        ...
        "mined": mined,
        "responded": responded,
        "mined_percentage": round((mined / total * 100), 2) if total > 0 else 0,
        "response_rate": round((responded / mined * 100), 2) if mined > 0 else 0
    }